...
class SafetyIntermediary:
    ...
    async def validate_pattern(self, pattern: Dict) -> bool:
        """Validate if a pattern is safe to execute"""
        if pattern["type"] == "command":
            # Check for dangerous commands
            dangerous_commands = ["rm", "sudo", "mv", "cp"]
            for cmd in dangerous_commands:
                if cmd in pattern["content"]:
                    logging.warning(f"Dangerous command detected: {cmd}")
                    return False
        return True
```
